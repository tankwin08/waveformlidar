...
#' With this information, you can calculate the energy percentile heights. Here, the percentile will be calculated from the top by defalut.
...
#' @param top is to tell whether we calculate the percentile from the top (where waveform starts or canopy) or from the bottom (where the waveform ends).
...
percentile.location<-function(x,quan=seq(0.5,1,0.1),rescale = TRUE, top = TRUE){
  ...
  if (top) {
    ...
    csum<-cumsum(x)
    sind<-sapply(csum, function(z) z>=qsum)
    ind0<- apply(sind,1, function(z) which(z==TRUE)[1])
    ind<- length(x) - ind0 +1
  }
  return(ind)
}
